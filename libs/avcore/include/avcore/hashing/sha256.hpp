...
/// @file sha256.hpp
/// @brief Self-contained streaming implementation of the SHA-256 digest
///        (FIPS 180-4).
...
class Sha256 {
  public:
    ...
    /// @brief Fixed-size SHA-256 digest.
    using Digest = std::array<std::uint8_t, digest_size>;

    /// @brief Construct a hasher initialised to the SHA-256 starting state.
    Sha256();
    ...
    /// @return The 32-byte SHA-256 digest of all absorbed data.
    Digest finalize();

    /// @brief One-shot helper hashing a whole buffer.
    /// @param data Bytes to hash.
    /// @return The SHA-256 digest of @p data.
